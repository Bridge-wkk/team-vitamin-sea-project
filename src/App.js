    ...
    if (loginUser && loginUser.id) {
      ...
      fetch(`http://localhost:3010/requests`)
        ...
        .then((allRequests) => {
          ...
          // ✅ 自分宛かつ未払い(unpaid)を抽出（receiverId基準に統一）
          const myUnpaidRequests = allRequests.filter(
            (req) =>
          ...
          if (urgent && !isWarningClosed) {
            ...
            const audio = new Audio(
              "https://actions.google.com/google_assistant/notifications/emergency_alert.mp3"
            );
            audio.play().catch(() => {});
          }
        });
        ...
        >
          <div style={{ display: "flex", alignItems: "center" }}>
            <div className="avatar">
              <img src={loginUser.icon} alt="icon" />
            </div>
        ...
        {hasPendingRequest && (
          ...
          >
            ...
            <span
              ...
              style={{ color: "#D11C1C", fontWeight: "bold" }}
            >
              支払いリクエストが届いています
            </span>
          </button>
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: "14px", margin: "15px 0" }}>
              {urgentRequest.requesterName}さんからの請求（
              {urgentRequest.amount.toLocaleString()}円）から一週間以上経過しています。
            </p>

            {/* ✅ requestId付きで飛ばす（どの請求を払うか曖昧にならない） */}
            <button
              onClick={() =>
                navigate(`/payrequest?requestId=${encodeURIComponent(urgentRequest.id)}`)
              }
              style={{
                width: "100%",
