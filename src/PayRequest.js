...
const PayRequest = ({ loginUser }) => {
  ...
  // ✅ ログインし直し後に戻ってくる先（=今開いているURL）
  const redirectTo = useMemo(
    () => `${location.pathname}${location.search}`,
    [location.pathname, location.search]
  );

  // 表示用データ（requestが取れたらそれを最優先）
  ...
  // 2) request取得 + 宛先チェック（receiverIdとloginUser.id）
  useEffect(() => {
    if (!loginUser) return;
    ...
    (async () => {
      try {
        if (requestId) {
          ...
          // ✅ 宛先チェック：本来の支払者(receiverId)とログインユーザーが違うならNotYourRequestへ
          if (String(data.receiverId) !== String(loginUser.id)) {
            navigate("/notyourrequest", {
              replace: true,
              state: {
                payerName: data.receiverName || "",      // 本来支払うべき人
                requesterName: data.requesterName || "", // 請求者
                redirectTo,                               // ログインし直し後に戻る先
              },
            });
            return;
          }
  ...
  }, [loginUser, requestId, navigate, redirectTo]);

  // 3) 請求者情報の取得（friendsから）
  useEffect(() => {
    if (!loginUser || invalidReason || !request) return;
  ...
  }, [loginUser, invalidReason, request, requesterId, requesterName]);
  ...
  const handleSend = async () => {
    ...
    if (!request) {
      alert("請求情報が取得できませんでした。");
      return;
    }

    // ✅ 二重防御：送金直前にも宛先チェック
    if (String(request.receiverId) !== String(loginUser.id)) {
      navigate("/notyourrequest", {
        replace: true,
        state: {
          payerName: request.receiverName || "",
